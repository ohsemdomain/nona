...
export function OrderFormPage() {
	...
	const { create, update } = useResource<
		Order,
		CreateOrderInput,
		UpdateOrderInput
	>("order", "Order");
	...
	// Combine form state for dirty tracking
	const formState = useMemo(() => ({ lineList, status }), [lineList, status]);
	const dirty = useFormDirty({
		form: formState,
		confirmId: "order-form-confirm-discard",
		blockNavigation: true,
	});

	// Store setOriginalForm in ref to avoid dependency issues
	const setOriginalFormRef = useRef(dirty.setOriginalForm);
	setOriginalFormRef.current = dirty.setOriginalForm;

	// Store setLineList in ref to avoid stale closure in inline modal callback
	const setLineListRef = useRef(setLineList);
	setLineListRef.current = setLineList;

	// Stable callback that always uses the latest setLineList
	const handleItemSuccess = useCallback((key: string, item: Item) => {
		setLineListRef.current((prev) =>
			prev.map((line) =>
	...
	// Inline modal hook
	const itemModal = useInlineModal<Item>(INLINE_ITEM_MODAL_ID, handleItemSuccess);

	useEffect(() => {
		if (existingOrder) {
			const loadedStatus = existingOrder.status;
			const loadedLineList = (existingOrder.lineList ?? []).map((line) => ({
				key: `existing-${line.id}`,
				itemId: String(line.itemId),
				quantity: line.quantity,
			}));
			setStatus(loadedStatus);
			setLineList(loadedLineList);
			setOriginalFormRef.current({ lineList: loadedLineList, status: loadedStatus });
		}
	}, [existingOrder]);
				...
				<div className="mx-auto max-w-3xl px-4 py-8">
					<div className="mb-6">
						<Button
							variant="secondary"
							size="sm"
							onClick={() => dirty.confirmClose(() => navigate("/order"))}
						>
